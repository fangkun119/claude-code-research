...
**代码**：Claude 通过 bash 运行的可执行脚本（fill\_form.py、validate.py）；脚本提供确定性操作而不消耗上下文
...
要了解更多信息，请参阅[使用 Claude API 的  Skills](/zh-CN/api/skills-guide)。

### Claude Code

[Claude Code](/zh-CN/docs/claude-code/overview) 仅支持自定义  Skills。

**自定义  Skills**：创建包含 SKILL.md 文件的目录形式的  Skills。Claude 自动发现并使用它们。

Claude Code 中的自定义  Skills 基于文件系统，不需要 API 上传。

要了解更多信息，请参阅[在 Claude Code 中使用  Skills](/zh-CN/docs/claude-code/skills)。

### Claude Agent SDK

[Claude Agent SDK](/zh-CN/api/agent-sdk/overview) 通过基于文件系统的配置支持自定义  Skills。
...
要了解更多信息，请参阅 [SDK 中的 Agent  Skills](/zh-CN/api/agent-sdk/skills)。
...
`description` 应包括  Skill  的功能以及 Claude 何时应使用它。有关完整的创作指导，请参阅[最佳实践指南](/zh-CN/docs/agents-and-tools/agent-skills/best-practices)。
...
这些  Skills 在 Claude API 和 claude.ai 上可用。请参阅[快速入门教程](/zh-CN/docs/agents-and-tools/agent-skills/quickstart)开始在 API 中使用它们。
...
* **仅预配置的依赖项**：检查[代码执行工具文档](/zh-CN/docs/agents-and-tools/tool-use/code-execution-tool)了解可用包的列表
